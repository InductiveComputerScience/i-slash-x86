package FormulaTranslation.BooleanFormulaSymbolicWriter;
...
public class BooleanFormulaSymbolicWriter{
	public static char [] ASTToBooleanFormula(ASTNode ast){
		char [] f, p1, p2;
		...
		if(ast.leaf){
			...
			f = AppendString(f, p1);
			f = AppendString(f, ")".toCharArray());
		}else if(StringsEqual(ast.value, "&".toCharArray()) || StringsEqual(ast.value, "|".toCharArray()) || StringsEqual(ast.value, "^".toCharArray()) || StringsEqual(ast.value, "=".toCharArray()) || StringsEqual(ast.value, "!=".toCharArray()) || StringsEqual(ast.value, "<".toCharArray()) || StringsEqual(ast.value, "<=".toCharArray()) || StringsEqual(ast.value, ">".toCharArray()) || StringsEqual(ast.value, ">=".toCharArray()) || StringsEqual(ast.value, "->".toCharArray()) || StringsEqual(ast.value, "<->".toCharArray())){
			f = new char [0];
			p1 = ASTToBooleanFormula(ast.l);
			p2 = ASTToBooleanFormula(ast.r);
			f = AppendString(f, p1);
			f = AppendString(f, " ".toCharArray());
	...
	public static void BooleanASTToTForm(ASTNode ast, StringReference tf, NumberReference t){
		double tl, tr;
		...
		if(!ast.leaf){
			if(StringsEqual(ast.value, "&".toCharArray()) || StringsEqual(ast.value, "|".toCharArray()) || StringsEqual(ast.value, "^".toCharArray()) || StringsEqual(ast.value, "=".toCharArray()) || StringsEqual(ast.value, "!=".toCharArray()) || StringsEqual(ast.value, "<".toCharArray()) || StringsEqual(ast.value, "<=".toCharArray()) || StringsEqual(ast.value, ">".toCharArray()) || StringsEqual(ast.value, ">=".toCharArray()) || StringsEqual(ast.value, "->".toCharArray()) || StringsEqual(ast.value, "<->".toCharArray())){
				if(!ast.l.leaf){
					BooleanASTToTForm(ast.l, tf, t);
					tl = t.numberValue;
				}

				if(!ast.r.leaf){
					BooleanASTToTForm(ast.r, tf, t);
					tr = t.numberValue;
				}
	...
	public static char [] BooleanFormulaToTForm(char [] f){
		char [] af;
		...
		if(success){
			...
			success = ParseBooleanTokens(tokens.stringArray, pos, ast, errorMessage);

			if(success){
				tf = CreateStringReference("".toCharArray());
				t = CreateNumberReference(0d);
				BooleanASTToTForm(ast, tf, t);
				af = tf.string;
			}else{
